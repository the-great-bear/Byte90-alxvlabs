...
bool NVSStorage::begin() {
    ...
    if (!_openai_prefs.begin(NS_OPENAI, false)) {
        ESP_LOGE(TAG, "❌ Failed to open OpenAI namespace");
        return false;
    }
...
// OpenAI API key
bool NVSStorage::saveOpenAiApiKey(const char* api_key) {
    if (!_initialized || !api_key) {
        return false;
    }

    if (!openNamespace(_openai_prefs, NS_OPENAI, false)) {
        return false;
    }

    bool success = (_openai_prefs.putString(KEY_OPENAI_API_KEY, api_key) > 0);
    closeNamespace(_openai_prefs);

    if (success) {
        ESP_LOGI(TAG, "OpenAI API key stored");
    } else {
        ESP_LOGE(TAG, "❌ Failed to store OpenAI API key");
    }

    return success;
}

bool NVSStorage::clearOpenAiApiKey() {
    if (!_initialized) {
        return false;
    }
    ...
    bool success = _openai_prefs.remove(KEY_OPENAI_API_KEY);
    closeNamespace(_openai_prefs);
...
bool NVSStorage::hasOpenAiApiKey() {
    if (!_initialized) {
        return false;
    }

    if (!openNamespace(_openai_prefs, NS_OPENAI, true)) {
        return false;
    }

    bool has_key = _openai_prefs.isKey(KEY_OPENAI_API_KEY);
    closeNamespace(_openai_prefs);

    return has_key;
}

String NVSStorage::getOpenAiApiKeyLast4() {
    if (!_initialized) {
        return "";
    }
...
String NVSStorage::getOpenAiApiKey() {
    if (!_initialized) {
        return "";
    }
