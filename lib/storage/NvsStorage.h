...
public:
    ...
    // MQTT configuration (Preferences-style simple access)

    /**
     * @brief Begin MQTT namespace for read/write
     *
     * @param readonly true for read-only access
     * @return true on success, false on failure
     */
    bool beginMqtt(bool readonly = false);

    /**
     * @brief End MQTT namespace
     */
    void endMqtt();

    /**
     * @brief Get MQTT Preferences object for direct access
     *
     * @return Reference to MQTT Preferences object
     */
    Preferences& getMqttPrefs() { return _mqtt_prefs; }
