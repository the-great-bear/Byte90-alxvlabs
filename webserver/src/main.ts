...
const openAiCardIcon = document.getElementById('openAiCardIcon') as HTMLSpanElement;
const openAiCard = document.getElementById('openAiCard') as HTMLDivElement;
const openAiNotification = document.getElementById('openAiNotification') as HTMLDivElement;
...
function setOpenAiNotification(message: string, type: StatusType = 'info') {
  if (!message) {
    openAiNotification.hidden = true;
    openAiNotification.textContent = '';
    return;
  }

  openAiNotification.hidden = false;
  openAiNotification.textContent = message;

  if (type === 'error' || type === 'danger') {
    openAiNotification.setAttribute('role', 'alert');
    openAiNotification.setAttribute('aria-live', 'assertive');
  } else if (type === 'warning') {
    openAiNotification.setAttribute('role', 'alert');
...
function updateButtons() {
  ...
  openAiApiKeyInput.readOnly = openAiHasKey || isOpenAiBusy;
  openAiApiKeyInput.disabled = isOpenAiBusy;
  openAiSaveBtn.disabled =
    openAiHasKey || openAiApiKeyInput.value.trim().length === 0;
...
async function fetchOpenAiJson(path: string, init?: RequestInit): Promise<OpenAiKeyResponse> {
  const response = await fetch(path, {
    cache: 'no-store',
    ...init,
    headers: {
  ...
  const data = JSON.parse(bodyText) as OpenAiKeyResponse;
  if (!response.ok || !data.success) {
    throw new Error(data.message || 'Request failed.');
  }
...
function updateOpenAiUi() {
  openAiCard.hidden = false;
  if (openAiHasKey) {
    openAiApiKeyInput.value = openAiLast4 ? `sk-******${openAiLast4}` : 'sk-******';
  } else {
...
async function fetchOpenAiStatus() {
  ...
  try {
    const data = await fetchOpenAiJson('/api/openai-key/status');
    openAiHasKey = data.has_key === true;
    openAiLast4 = typeof data.last4 === 'string' ? data.last4 : '';
    updateOpenAiUi();
  } catch (error) {
    console.error('OpenAI status check failed:', error);
    setOpenAiNotification(
      error instanceof Error ? error.message : 'OpenAI key status failed.',
      'error'
    );
  } finally {
    isOpenAiBusy = false;
...
async function saveOpenAiKey() {
  if (isOpenAiBusy || openAiHasKey) {
    return;
  }

  const apiKey = openAiApiKeyInput.value.trim();
  if (!apiKey) {
    setOpenAiNotification('API key is required.', 'warning');
    return;
  }

  isOpenAiBusy = true;
  setOpenAiNotification('Saving API key...', 'info');
  updateButtons();

  try {
    const data = await fetchOpenAiJson('/api/openai-key', {
      method: 'POST',
    ...
    console.error('OpenAI key save failed:', error);
    setOpenAiNotification(
...
async function clearOpenAiKey() {
  ...
  isOpenAiBusy = true;
  setOpenAiNotification('Clearing API key...', 'info');
  updateButtons();

  try {
    const data = await fetchOpenAiJson('/api/openai-key/clear', { method: 'POST' });
    openAiHasKey = false;
    openAiLast4 = '';
    openAiApiKeyInput.value = '';
    setOpenAiNotification(
    ...
    console.error('OpenAI key clear failed:', error);
    setOpenAiNotification(
...
function initialize() {
  ...
  openAiSaveBtn.addEventListener('click', () => {
    runWithButtonFocus(openAiSaveBtn, saveOpenAiKey);
  });
