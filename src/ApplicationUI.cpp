...
static constexpr int ACTIVATION_BODY_TEXT_SIZE = 3;
static constexpr int ACTIVATION_SCROLL_STEP_PX = 2;
static constexpr unsigned long ACTIVATION_SCROLL_INTERVAL_MS = 60;
static constexpr int ACTIVATION_SCROLL_GAP_PX = 24;
...
ApplicationUI::ApplicationUI(LittleFSManager* fs, ArduinoSSD1351* display)
    ...
    , _activation_marquee_dirty(false)
    , _activation_scroll_offset(0)
    , _activation_scroll_last_ms(0)
...
)
{
    ...
    if (_showing_activation) {
        int max_width = DISPLAY_WIDTH - 12;
        int text_width = static_cast<int>(_activation_code.length()) *
                         6 * ACTIVATION_BODY_TEXT_SIZE;
        if (text_width > max_width) {
            unsigned long now = millis();
            if ((now - _activation_scroll_last_ms) >= ACTIVATION_SCROLL_INTERVAL_MS) {
                int cycle = text_width + ACTIVATION_SCROLL_GAP_PX;
                _activation_scroll_offset =
                    (_activation_scroll_offset + ACTIVATION_SCROLL_STEP_PX) % cycle;
                _activation_scroll_last_ms = now;
                _activation_marquee_dirty = true;
            }
        } else if (_activation_scroll_offset != 0) {
            _activation_scroll_offset = 0;
            _activation_marquee_dirty = true;
        }
        ...
        if (xSemaphoreTake((SemaphoreHandle_t)_display_mutex, pdMS_TO_TICKS(50)) == pdTRUE) {
            ...
            } else if (_activation_marquee_dirty) {
                drawActivationCodeRow(_activation_code);
            }
    ...
    if (!charging_animation_active &&
        ...
        motion_animation_active = _motion_animator->update(current_state, true);
...
void ApplicationUI::showActivation(const String& url, const String& code)
{
    ...
    _activation_marquee_dirty = false;
    _activation_scroll_offset = 0;
    _activation_scroll_last_ms = millis();
    _animate_active = false;
...
void ApplicationUI::playRandomEmotionOnce()
{
    if (_showing_activation || _showing_clock || !_gif_manager) {
        return;
    }
    _gif_manager->playRandomEmotionOnce();
}
...
void ApplicationUI::drawActivationScreen(const String& url, const String& code)
{
    ...
    gfx->print("OpenAI API");

    int16_t tmp_x, tmp_y;
    uint16_t tmp_w, label_h;
    gfx->getTextBounds("OpenAI API", 0, 0, &tmp_x, &tmp_y, &tmp_w, &label_h);

    drawActivationCodeRow(code);
    ...
    if (display_url.length() > 0) {
        ...
        gfx->setTextSize(1);
        int16_t url_y = content_top + (int)label_h + 8 +
                        (8 * ACTIVATION_BODY_TEXT_SIZE) + 8;
        gfx->setCursor(6, url_y);
        gfx->print(display_url);
    }
}

void ApplicationUI::drawActivationCodeRow(const String& code)
{
    if (!_display) return;
    ...
    int row_h = 8 * ACTIVATION_BODY_TEXT_SIZE;
    gfx->fillRect(0, code_y - 1, DISPLAY_WIDTH, row_h + 2, COLOR_BLACK);

    int max_width = DISPLAY_WIDTH - 12;
    int text_width = static_cast<int>(code.length()) * 6 * ACTIVATION_BODY_TEXT_SIZE;
    gfx->setTextColor(COLOR_WHITE);
    gfx->setTextSize(ACTIVATION_BODY_TEXT_SIZE);
    ...
    int scroll_x = code_x - static_cast<int>(_activation_scroll_offset);
    int cycle = text_width + ACTIVATION_SCROLL_GAP_PX;
    gfx->setCursor(scroll_x, code_y);
    gfx->print(code);
    gfx->setCursor(scroll_x + cycle, code_y);
    gfx->print(code);
}
