...
InputManager::InputManager(
    ...
    , _openai_key_prompt_dismissed(openai_key_prompt_dismissed)
...
void InputManager::handleButtonClick(const Event&) {
    ...
    if (_ui && _ui->isShowingActivation()) {
        _openai_key_prompt_dismissed = true;
        _ui->clearActivation();
        ESP_LOGI(TAG, "Activation prompt dismissed by button");
        return;
    }

    if (_ui && _ui->isShowingClock() &&
        _state_manager && _state_manager->getState() == SYSTEM_STATE_IDLE) {
        bool is_connected = _protocol_ready;
        if (!is_connected) {
            _ui->clearClock();
            return;
        }
    ...
    if (_state_manager && _state_manager->getState() == SYSTEM_STATE_SPEAKING) {
        if (_event_bus) {
            _event_bus->publish({EventType::CANCEL_OPENAI_RESPONSE, "user_stopped"});
        }
        if (_audio) {
    ...
    if (!_audio || !_audio->isListening()) {
        ...
        if (!is_connected) {
            ...
            if (_event_bus) {
                _event_bus->publish({EventType::START_LISTENING, "button"});
            }
