...
void McpToolManager::setApiClient(ApiClient* client) {
    _api_client = client;
}
...
void McpToolManager::runToolWorker() {
    while (true) {
        ...
        if (_tool_queue &&
            ...
            if (!response.isEmpty() && _api_client) {
                ...
                if (!error) {
                    ...
                    ESP_LOGI(TAG, "MCP tool response sent");
                    _api_client->sendMcpResponse(request->session_id, response_doc.as<JsonObject>());
                } else {
