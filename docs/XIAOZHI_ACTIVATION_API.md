# Device Activation System Documentation

## Overview

The activation system requires devices to be activated with the server before full operation. It uses a challenge-response authentication scheme with HMAC-SHA256 when a hardware serial number is available, and falls back to a V1 payload when it is not.
